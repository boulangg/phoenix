...
int unwind_get_frame_description_entry(uint64_t ip, frame_description_entry_t* outFde)
{
    ...
    DEBUG_LOG("FDE (0x%08x -> 0x%08x) found for ip 0x%08x\n", outFde->ipStart, outFde->ipEnd, ip);
    return 0;
}

int unwind_parse_cfi(common_information_entry_t cie, uint64_t start, uint64_t end, uint64_t base, uint64_t ip,
                     cfa_rules_t initialRules, cfa_rules_t* rules)
{
    uint64_t addr = start;
    int8_t op;
    ...
    while (addr < end && base < ip && (op = unwind_dwarf_read_u8(&addr)) != DW_CFA_nop) {
        if ((op & DW_CFA_base_op_mask) == DW_CFA_advance_loc) {
            ...
            rules->rules[reg] =
                (cfa_rule_t){.type = CFA_RULE_OFFSET, .reg = reg, .offset = offset * cie.dataAlignmentFactor};
            continue;
        ...
        case DW_CFA_def_cfa_sf:
            ...
            rules->cfa = (cfa_rule_t){
                .type = CFA_RULE_CFA_REGISTER_OFFSET, .reg = reg, .offset = s_offset * cie.dataAlignmentFactor};
            continue;
...
int unwind_parse_cfa_rules(frame_description_entry_t fde, uint64_t ip, cfa_rules_t* outRules)
{
    ...
    unwind_parse_cfi(fde.cie, fde.cie.callFrameInstrStart, fde.cie.callFrameInstrEnd, fde.ipStart, ip, initalRules,
                     &initalRules);

    memcpy(outRules, &initalRules, sizeof(initalRules));
    unwind_parse_cfi(fde.cie, fde.callFrameInstrStart, fde.callFrameInstrEnd, fde.ipStart, ip, initalRules, outRules);
