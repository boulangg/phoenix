...
VirtualMapping::VirtualMapping() :
    pageTable(nullptr), entryPoint(nullptr), startCode(nullptr), endCode(nullptr), startData(nullptr), endData(nullptr),
    topStack(nullptr), startStack(nullptr), startBrk(nullptr), currBrk(nullptr)
{
    // Add syscall stack
    {
        uint64_t prot = VirtualMapping::PROT::EXEC | VirtualMapping::PROT::WRITE | VirtualMapping::PROT::READ;
        uint64_t flags = VirtualMapping::FLAGS::PRIVATE | VirtualMapping::FLAGS::ANONYMOUS |
                         VirtualMapping::FLAGS::EXECUTABLE | VirtualMapping::FLAGS::FIXED |
                         VirtualMapping::FLAGS::POPULATE | VirtualMapping::FLAGS::KERNEL;
    ...
    // Add stack
    {
        uint64_t prot = VirtualMapping::PROT::EXEC | VirtualMapping::PROT::WRITE | VirtualMapping::PROT::READ;
        uint64_t flags = VirtualMapping::FLAGS::PRIVATE | VirtualMapping::FLAGS::ANONYMOUS |
                         VirtualMapping::FLAGS::EXECUTABLE | VirtualMapping::FLAGS::GROWNSDOWN |
...
uint64_t* VirtualMapping::mmap(uint64_t* addr, uint64_t len, uint64_t prot, uint64_t flags, File* file, uint64_t offset)
{
...
std::list<VirtualArea*>::iterator VirtualMapping::mergeSurroundingAreas(std::list<VirtualArea*>::iterator curr)
{
