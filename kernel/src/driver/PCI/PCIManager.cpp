...
uint32_t PCIManager::getAddress(uint8_t bus, uint8_t slot, uint8_t func, uint8_t off)
{
    ...
    address = ((uint32_t)0x80000000) | (lbus << 16) | (lslot << 11) | (lfunc << 8) | (loff);
...
int PCIManager::matchDeviceDriver(PCIDevice* device, PCIDriver* driver)
{
    ...
    for (auto deviceID : deviceIDs) {
        if ((deviceID.vendorID != (uint32_t)PCI_ANY_ID) && (deviceID.vendorID != device->getVendorID())) {
            continue;
        }
        if ((deviceID.deviceID != (uint32_t)PCI_ANY_ID) && (deviceID.deviceID != device->getDeviceID())) {
            return -1;
        }
        if ((deviceID.subVendorID != (uint32_t)PCI_ANY_ID) && (deviceID.subVendorID != device->getSubVendorID())) {
            return -1;
        }
        if ((deviceID.subDeviceID != (uint32_t)PCI_ANY_ID) && (deviceID.subDeviceID != device->getSubDeviceID())) {
            return -1;
        }
