...
Process::Process(mem::PageTable pageTable, std::uint64_t entryPoint) : _memDesc(pageTable)
{
...
void Process::initUserProcess(const char* argv[], const char* envp[], auxv_t auxv[])
{
    ...
    std::size_t paramSize =
        (1 + 1 + 1 + argc + 1 + envc + 1) * sizeof(std::uint64_t) + auxc * sizeof(auxc) + sizeof(std::uint64_t);
