...
VirtualArea::VirtualArea(uint64_t* addrStart, uint64_t* addrEnd, uint64_t flags, File* file, uint64_t offset,
                         uint64_t fileSize)
{
    ...
    uint64_t nbPages = (((uint64_t)addrEnd) + PAGE_SIZE - 1) / PAGE_SIZE - ((uint64_t)addrStart) / PAGE_SIZE;
    if ((flags & FLAGS::VM_WRITE) && !(flags & FLAGS::VM_SHARED)) {
...
{
    return (((uint64_t)addrEnd) + PAGE_SIZE - 1) / PAGE_SIZE - ((uint64_t)addrStart) / PAGE_SIZE;
}
...
bool VirtualArea::tryMergeArea(VirtualArea* area)
{
    ...
    if ((this->addrEnd != area->addrStart) & (this->file == area->file)) {
        return false;
    }
