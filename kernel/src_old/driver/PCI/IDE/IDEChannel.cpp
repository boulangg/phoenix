...
IDEChannel::IDEChannel(IDEDevice* device, IDEChannelRegisters regs, int channelNo) :
    device(device), base(regs.base), ctrl(regs.ctrl), bmIDE(regs.bmIDE), nIEN(regs.nIEN), channelNo(channelNo)
{
    initDrives();
}
        ...
        for (std::size_t i = 0; i < bio.nb_sectors; ++i) {
            if (DMA_ENABLED && this->supportsDMA()) {
                ...
                InterruptHandler* handler =
                    new InterruptHandlerFunction<IDEChannel::handler>("IDE", {true, false}, nullptr);
...
std::uint8_t IDEChannel::ATAAccessPIO(bool slave, bool write, std::uint64_t lba, char* buffer, std::uint32_t numSec)
{
    ...
    char cache_cmd[] = {ATA_CMD_CACHE_FLUSH, ATA_CMD_CACHE_FLUSH, ATA_CMD_CACHE_FLUSH_EXT};
    if (write == 0) {
        // PIO Read.
...
std::uint8_t IDEChannel::ATAAccessDMA(bool slave, bool write, std::uint64_t lba, uint64_t physAddr,
                                      std::uint32_t numSec)
{
...
std::uint8_t IDEChannel::sendCommand(bool slave, bool write, std::uint64_t lba, std::uint32_t numSec, bool isDMA)
{
    ...
    // disks[slave]->sectorSize, disks[slave]->getSize()); Console::write(tmp);
    ...
    if (lba_mode == 0 && isDMA == 0 && write == 0)
        cmd = ATA_CMD_READ_PIO;
    if (lba_mode == 1 && isDMA == 0 && write == 0)
        cmd = ATA_CMD_READ_PIO;
    if (lba_mode == 2 && isDMA == 0 && write == 0)
        cmd = ATA_CMD_READ_PIO_EXT;
    if (lba_mode == 0 && isDMA == 1 && write == 0)
        cmd = ATA_CMD_READ_DMA;
    if (lba_mode == 1 && isDMA == 1 && write == 0)
        cmd = ATA_CMD_READ_DMA;
    if (lba_mode == 2 && isDMA == 1 && write == 0)
        cmd = ATA_CMD_READ_DMA_EXT;
    if (lba_mode == 0 && isDMA == 0 && write == 1)
        cmd = ATA_CMD_WRITE_PIO;
    if (lba_mode == 1 && isDMA == 0 && write == 1)
        cmd = ATA_CMD_WRITE_PIO;
    if (lba_mode == 2 && isDMA == 0 && write == 1)
        cmd = ATA_CMD_WRITE_PIO_EXT;
    if (lba_mode == 0 && isDMA == 1 && write == 1)
        cmd = ATA_CMD_WRITE_DMA;
    if (lba_mode == 1 && isDMA == 1 && write == 1)
        cmd = ATA_CMD_WRITE_DMA;
    if (lba_mode == 2 && isDMA == 1 && write == 1)
